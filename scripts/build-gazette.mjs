...
//   cd press && npm install --omit=dev   (once — fetch the bundler's inputs)
...
if (!existsSync(join(gz, "node_modules"))) {
  console.error("✗ press/node_modules missing — run `cd press && npm install --omit=dev` first.");
  process.exit(1);
}
...
if (!existsSync(esbuild)) {
  console.error("✗ esbuild not in press/node_modules — run `cd press && npm install --omit=dev` first.");
  process.exit(1);
}

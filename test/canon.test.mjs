...
test("bureau's own canon builds healthy (self-dogfood)", (t) => {
  if (!existsSync(CANON)) return t.skip("no canon/ workspace");
  const out = mkdtempSync(join(tmpdir(), "canon-board-"));
  execFileSync("node", [GAZETTE, "build", "--dir", CANON, "--out", out], { cwd: ROOT, stdio: "ignore" });
  const h = execFileSync("node", [GAZETTE, "health", "--dir", CANON], { cwd: ROOT, encoding: "utf8" });

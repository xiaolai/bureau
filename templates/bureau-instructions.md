...
- **The decision log is the source of truth.** `canonical` is a **projection** of a logged
  ...
  approval backs. Never hand-edit `{{WORKSPACE}}/_log.jsonl`; it is append-only, and its hash chain detects naive edits
(it is unsigned, so not proof against a determined rewrite).
...
- **Mechanical, code-owned** (never hand-write) — run the bundled press,
  `node "${CLAUDE_PLUGIN_ROOT}/press/bin/gazette.mjs" <verb> --dir {{WORKSPACE}}`. **Invoke it that
  way exactly**: `${CLAUDE_PLUGIN_ROOT}` is this plugin's own install directory — pass the variable

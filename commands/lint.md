...
free-text contradictions between pages, claims a later session superseded, undocumented gaps,
...
2. Read every cabinet drawer (exclude `logbook/` and `board/`). If there are no cabinet pages,
   report "no cabinets to lint" and stop.
3. For each finding type — contradiction, superseded, gap, drift — find candidates, then
   adversarially refute each one and keep only the survivors (a false finding erodes trust).
   Weight the sweep by the active `bureau.json` profiles.
